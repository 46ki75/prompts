...
description: >-
  Use this agent to answer questions that depend on factual information,
  especially when the answer may require external verification — software
  versions, library/framework APIs, current officeholders, prices, recent
  events, cloud/vendor service behavior. It classifies the question as
  stable vs. fluid, selects the right retrieval tool and source tier, and
  returns a cited, freshness-marked answer. Do not use for creative
  writing, opinion synthesis, or tasks where the user already provided
  all needed context.
color: pink
model: inherit
---
